# Árboles binarios AB y ABB | Semana 15 | Jesús Alberto Domínguez Charris
...
class NodosBinariosConColores(NodosBinarios):
    def __init__(self, dato, nodoIzquierdo=None, nodoDerecho=None, color="") -> None:
        super().__init__(dato, nodoIzquierdo, nodoDerecho)
        self.color = color

    def __str__(self) -> str:
        return f"{(self.valorNodo, self.color)}"
    ...
    def __verArbolRecursivo(self, arbol: "NodosBinariosConColores", recorrido: str, nivel = 0) -> str:
        if arbol is None:
            return ""
        espaciado = "\t" * nivel
        recorrido = espaciado + f"{(arbol.valorNodo, arbol.color)}\n" \
            + f"{self.__verArbolRecursivo(arbol.hijoIzquierdo, recorrido, nivel + 1)}" \
            + f"{self.__verArbolRecursivo(arbol.hijoDerecho, recorrido, nivel + 1)}" \
            + recorrido
        return recorrido
...
class ArbolesBinariosBusqueda:
    ...
    """
    def verNodosTresMayores(self):
        if not self.estaVacio() and self.nodoRaiz.contarNodosArbol() >= 3:
            listaNodos = self.nodoRaiz.verNodosArbol()
            listaValoresNodos = []
            nodo: NodosBinarios
            for nodo in listaNodos:
                listaValoresNodos.append(nodo.valorNodo)

            maximo_1 = listaValoresNodos[0]
            maximo_2 = listaValoresNodos[0]
            maximo_3 = listaValoresNodos[0]
            index_1 = 0
            index_2 = 0
            index_3 = 0

            maximo_1 = max(listaValoresNodos)
            index_1 = listaValoresNodos.index(maximo_1)
            listaValoresNodos.pop(index_1)

            maximo_2 = max(listaValoresNodos)
            index_2 = listaValoresNodos.index(maximo_2)
            listaValoresNodos.pop(index_2)

            maximo_3 = max(listaValoresNodos)
            index_3 = listaValoresNodos.index(maximo_3)
            listaValoresNodos.pop(index_3)

            return maximo_1, maximo_2, maximo_3
    
    def verPadresNodosTresMayores(self):
        if not self.estaVacio() and self.nodoRaiz.contarNodosArbol() >= 3:
            tresMayores = self.verNodosTresMayores()
            padre_1 = self.verPadreNodo(tresMayores[0])
            padre_2 = self.verPadreNodo(tresMayores[1])
            padre_3 = self.verPadreNodo(tresMayores[2])
            return padre_1, padre_2, padre_3

    """
    Para un árbol binario de búsqueda, 
    indicar el número de nodos a visitar desde la raíz para llegar a cada nodo hoja.
    """
    def contarNodosHastaCadaHoja(self):
        listaHojas = self.nodoRaiz.verHojasArbol()
        tuplaNodosContados = list()
        hoja: NodosBinarios
        for hoja in listaHojas:
            tuplaNodosContados.append((hoja, hoja.profundidadNodo(self.nodoRaiz)))
        return tuplaNodosContados
    ...
    """
    def insertarNodoConColor(self, valorNumerico: int, colorNodo: str):
        if self.estaVacio():
            self.nodoRaiz = NodosBinariosConColores(valorNumerico, color = colorNodo)
        self.__insertarNodoConColorRecursivo(self.nodoRaiz, valorNumerico, colorNodo)
        
    def __insertarNodoConColorRecursivo(self, arbol: NodosBinariosConColores, valorNumerico: int, colorNodo: str):
        if valorNumerico == arbol.valorNodo and colorNodo == arbol.color:
            return
        if valorNumerico < arbol.valorNodo:
            if arbol.hijoIzquierdo is None:
                arbol.hijoIzquierdo = NodosBinariosConColores(valorNumerico, color = colorNodo)
            else:
                self.__insertarNodoConColorRecursivo(arbol.hijoIzquierdo, valorNumerico, colorNodo)
        else:
            if arbol.hijoDerecho is None:
                arbol.hijoDerecho = NodosBinariosConColores(valorNumerico, color = colorNodo)
            else:
                self.__insertarNodoConColorRecursivo(arbol.hijoDerecho, valorNumerico, colorNodo)    

    def sumaNodosColorEspecifico(self, color: str):
        if not self.estaVacio():
            listaNodos = self.nodoRaiz.verNodosArbol()
            listaNodosColorUsuario = list()
            nodo: NodosBinariosConColores
            for nodo in listaNodos:
                if nodo.color == color:
                    listaNodosColorUsuario.append(nodo)

            if len(listaNodosColorUsuario) != 0:
                suma = 0
                for nodo in listaNodosColorUsuario:
                    suma = suma + nodo.valorNodo
                return suma
        return None
